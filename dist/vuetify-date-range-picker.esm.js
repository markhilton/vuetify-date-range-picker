...
import { VIcon, VCol, VRow, VSheet, VDatePicker, VTextField, VBtn, VCheckbox, VCardText, VSpacer, VCardActions, VCard, VContainer, VDialog, VOverlay } from 'vuetify/lib';
  ...
  computed: {
    getDateStart: function getDateStart() {
      return this.dateStart
        ? moment(this.dateStart).format(DateRangePresets.DEFAULT_FORMAT)
        : moment(DateRangePresets.LAST_7_DAYS[0]).format(DateRangePresets.DEFAULT_FORMAT)
    },

    getDateUntil: function getDateUntil() {
      return this.dateUntil
        ? moment(this.dateUntil).format(DateRangePresets.DEFAULT_FORMAT)
        : moment(DateRangePresets.LAST_7_DAYS[1]).format(DateRangePresets.DEFAULT_FORMAT)
    },

    getCompareStart: function getCompareStart() {
      return this.compareStart
        ? moment(this.compareStart).format(DateRangePresets.DEFAULT_FORMAT)
        : moment(DateRangePresets.PREVIOUS_PERIOD([this.getDateStart, this.getDateUntil])[0]).format(DateRangePresets.DEFAULT_FORMAT)
    },

    getCompareUntil: function getCompareUntil() {
      return this.compareUntil
        ? moment(this.compareUntil).format(DateRangePresets.DEFAULT_FORMAT)
        : moment(DateRangePresets.PREVIOUS_PERIOD([this.getDateStart, this.getDateUntil])[1]).format(DateRangePresets.DEFAULT_FORMAT)
    },
  }
};
...
var script$1 = {
  ...
  watch: {
    ...
    pickerMainLeft: function pickerMainLeft(val) {
      this.pickerMainRight = moment(val).add(1, "month").format(DateRangePresets.MONTH_FORMAT);
    },

    pickerMainRight: function pickerMainRight(val) {
      this.pickerMainLeft = moment(val).subtract(1, "month").format(DateRangePresets.MONTH_FORMAT);
    },

    // The compare date picker should display the same month as the primary one
    pickerCompareLeft: function pickerCompareLeft(val) {
      this.pickerCompareRight = moment(val).add(1, "month").format(DateRangePresets.MONTH_FORMAT);
    },

    pickerCompareRight: function pickerCompareRight(val) {
      this.pickerCompareLeft = moment(val).subtract(1, "month").format(DateRangePresets.MONTH_FORMAT);
    },
  },
  ...
  mounted: function mounted() {
    ...
    if (config) {
      ...
      this.pickerMainLeft = moment(config.dateStart).subtract(1, "month").format(DateRangePresets.MONTH_FORMAT);
      this.pickerMainRight = moment(config.dateStart).format(DateRangePresets.MONTH_FORMAT);
    }

    // TO DO: set default to default preset instead hard coded
    // in case something weird happens and some defaults are needed
    else {
      this.pickerMainLeft = moment().subtract(1, "month").format(DateRangePresets.MONTH_FORMAT);
      this.pickerMainRight = moment().format(DateRangePresets.MONTH_FORMAT);
  ...
  methods: {
    ...
    setComparePreviousYear: function setComparePreviousYear() {
      ...
      this.pickerMainLeft = this.pickerCompare[0];
      this.pickerCompareLeft = moment(this.pickerMain[0]).subtract(1, "year").format(DateRangePresets.MONTH_FORMAT);
